...
for i in 1; do
    for PROMPTS in 1 3 4; do
        for TEMPERATURE in 10; do
            for N_REPEATS in 1 5 10 20 40; do
                for GENERATOR in gpt4o llama2; do
                    MODEL_NAME="llama2_7b_chat_rephrasing_prompts${PROMPTS}_repeat${N_REPEATS}_temperature${TEMPERATURE}_${GENERATOR}_${i}"
                    ...
                    if [ ! -d "${OUTPUT_PATH}" ]; then
                        torchrun --nnodes=1 --nproc-per-node=${N_GPUS} scripts/train.py \
                            ...
                            --dataset_config_name "rephrasing-prompts_${PROMPTS}-repeat_${N_REPEATS}-temperature_${TEMPERATURE}-${GENERATOR}" \
