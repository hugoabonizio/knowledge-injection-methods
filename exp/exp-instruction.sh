...
for i in 1; do
    for N_REPEATS in 1 5 10 20 40; do
        ...
        MODEL_NAME="llama2_7b_chat_instruction_repeat${N_REPEATS}-temperature_${TEMPERATURE}_${i}"
        ...
        if [ ! -d "${OUTPUT_PATH}" ]; then
            torchrun --nnodes=1 --nproc-per-node=${N_GPUS} scripts/train.py \
                ...
                --dataset_config_name "instruction-1_shot-repeat_${N_REPEATS}-temperature_${TEMPERATURE}" \
